...
class ConectionInsurer extends Component {
  ...
  constructor(props){
    super(props)
    this.state = {
      influencerName: "Vanesso Pudinic",
      solicitar: false,
      clicked: false,
      name: '',
      cpf: '',
    ...
    if (params.insurer == "life") {
      return <div className="CadastroAPI">
        <div className="CadastroAPI-title">
          Informações necessárias para a cotação de Vida
        </div>
      ...
      return  <Fragment>
        <div className="CadastroAPI">
          <div className="CadastroAPI-title">
            Informações necessárias para a cotação Viagem
          </div>
          <div style={{display: `flex`, justifyContent: 'center'}}>
            <div style={{display: 'flex', flexDirection: 'column', margin: '0px 16px'}}>        
              <input value={"Vanesso Pudinic"} />
              <input value={"Vanesso Pudinic"} />          
            </div>
            <div style={{display: 'flex', flexDirection: 'column', margin: '0px 16px'}}>
              <input placeholder={"BR"} value={"BR"}/>
              <div className="CadastroAPI-submit" onClick={()=> {
                this.setState({clicked: true})
                this.props.postTrip().then(()=> {
                  this.setState({solicitar: true})
                })
              }}>
                Solicitar cotação
